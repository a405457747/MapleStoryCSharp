...
public static class AutoCreateUI
{
    ...
    static void CreateUI()
    {
       ...
       // Path.Combine(Application.dataPath, "MapleStory/UI/Editor/SendObj.txt");
        ...
        if (Regex.IsMatch(goName, ".+Panel") || Regex.IsMatch(goName, ".+GameObject"))
        {
            ...
            List<Transform> transforms =  FindWantTransform(selectedObject);// selectedObject.GetComponentsInChildren<Transform>().ToList();transforms.Remove(selectedObject.transform);
            ...
            foreach (var trans in transforms)
            {
                ...
                if ((transNameCanMatch(transName) == true) && nameTypeData.ContainsKey(transName) == false)
                {
                    ...
                    nameTypeData.Add(transName, nameType);
                    //Debug.Log(transName + "：" + nameType);
                }
            }
            ...
            //Debug.Log(sendObjJson+":"+sendObj.data1.Count);
            //ExecutePythonScript("AutoCreate.py"+" "+sendObjJson );
