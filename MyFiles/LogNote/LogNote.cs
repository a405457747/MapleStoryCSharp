...
namespace MapleStory
{
    public class LogNote
    {
        ...
        public static bool NeedTimeMessage = true;
        ...
        static string GetDateTimeNowMessage()
        {
            if (NeedTimeMessage) return DateTime.Now.ToLocalTime().ToString();
            return "";
        }
        ...
        public static void Debug(params object[] paras)
        {
            ...
            var res = $"{ColorLabel($"[{GetDateTimeNowMessage()}][Debug] ", "#38B0DE")}{LogPrint(paras)}";
            if (LogLv >= 4) UnityEngine.Debug.Log(res);
        }

        public static void Info(params object[] paras)
        {
            var res = $"{ColorLabel($"[{GetDateTimeNowMessage()}][Info] ", "#66ff00")}{LogPrint(paras)}";
            if (LogLv >= 3) UnityEngine.Debug.Log(res);
        }

        public static void Warning(params object[] paras)
        {
            var res = $"{ColorLabel($"[{GetDateTimeNowMessage()}][Warning] ", "#FFFF00")}{LogPrint(paras)}";
            if (LogLv >= 2) UnityEngine.Debug.Log(res);
        }

        public static void Error(params object[] paras)
        {
            var res = $"{ColorLabel($"[{GetDateTimeNowMessage()}][Error] ", "#FF0000")}{LogPrint(paras)}";
            if (LogLv >= 1) UnityEngine.Debug.Log(res);
        }

        public static void Critical(params object[] paras)
        {
            var res = $"{ColorLabel($"[{GetDateTimeNowMessage()}][Critical] ", "#FF0000")}{LogPrint(paras)}";
