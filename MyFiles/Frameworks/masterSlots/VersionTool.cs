...
public class VersionTool : EditorWindow
{
    ...
    private static void StartBuild()
    {
        ...
        var pConfigList = Resources.Load<ProductConfigList>("ProductConfigList");
        var configIos = pConfigList.list[0];
        var configGoogle = pConfigList.list[1];
        var configAmazon = pConfigList.list[2];

        if (pConfigList.list[0].LandScape)
        ...
        if (Differences.Android())
        {
            ...
            {
                PlayerSettings.companyName = configAmazon.CompanyName;
                PlayerSettings.productName = configAmazon.ProductName;

                applicationIdentifier = $"com.amazon.{PlayerSettings.companyName}.{PlayerSettings.productName}";

                applicationIdentifier = "com.superwinslots.amazon.slotsmaster";
                pkgType = PackageType.apk;
                buildTarget = BuildTarget.Android;
            }
            else if (data.androidChannel == AndroidChannel.Google)
            {
                PlayerSettings.companyName = configGoogle.CompanyName;
                PlayerSettings.productName = configGoogle.ProductName;

                applicationIdentifier = $"com.{PlayerSettings.companyName}.{PlayerSettings.productName}";
                applicationIdentifier = "com.superwinslots.gp.slotsmaster";
                if (data.OpenAPK)
                {
                    pkgType = PackageType.apk;
                }
        ...
        {
            PlayerSettings.companyName = configIos.CompanyName;
            PlayerSettings.productName = configIos.ProductName;

            applicationIdentifier = $"com.{PlayerSettings.companyName}.{PlayerSettings.productName}";
            applicationIdentifier = "com.superwinslots.ios.slotsmaster";
