...
public class ResourceAssetProxyFactory : IAssetFactory
{
    ...
    private readonly Dictionary<string, SpriteAtlas> m_Atlas;
    ...
    public Sprite[] LoadPS(int lv)
    {
        Sprite[] res = Resources.LoadAll<Sprite>($"Sprite/Paytable{lv}");
        return res;
    }
    public ResourceAssetProxyFactory()
    {
        ...
        m_Atlas = new Dictionary<string, SpriteAtlas>();
    }

    public override Sprite LoadAtlasSprite(string atlasName, string spriteName)
    {
        spriteName = spriteName.Trim();
        if (m_Atlas.ContainsKey(atlasName) == false)
        {
            m_Atlas.Add(atlasName, Resources.Load<SpriteAtlas>($"Sprite/{atlasName}"));
        }

        return m_Atlas[atlasName].GetSprite(spriteName);
    }
